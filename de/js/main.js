$(document).ready(function() {
	...
	sliderAge.noUiSlider.on('update', function() {
		...
		if ( filterInput['ageFrom'] === 0 ) {
			agemin.innerHTML = agemin.innerHTML.replace(/.{1,}/, 'unbestimmt');
		} else {
			agemin.innerHTML = agemin.innerHTML.replace(/.{1,}/, 'ab ' + filterInput['ageFrom'] + ' Jahren');
		}

		if ( filterInput['ageTo'] === 0 ) {
			agemax.innerHTML = agemax.innerHTML.replace(/.{1,}/, 'unbestimmt');
		} else {
			agemax.innerHTML = agemax.innerHTML.replace(/.{1,}/, 'bis ' + filterInput['ageTo'] + ' Jahre');
		}
	})
